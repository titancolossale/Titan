# =====================================
# Phase 5.2 — Cinematic Living Intelligence screenshots
# =====================================
"""Capture cinematic living intelligence screenshots without starting the Titan API.
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        if "tdl-v2-orchestrator-idle-life" not in root_html:
            raise RuntimeError("Orchestrator idle life layer missing")

        phase = page.locator("#titan-v2-root").get_attribute("data-phase")
        if phase not in ("5.2", "5.1", "5"):
            raise RuntimeError(f"Expected root data-phase 5.2, got {phase!r}")

        _pin_idle(page)
        page.wait_for_timeout(1200)

        page.screenshot(
            path=str(OUT / "phase-5.2-cinematic-full.png"),
            full_page=False,
        )

        center = page.locator(".tdl-v2-center-stack")
        center.screenshot(path=str(OUT / "phase-5.2-cinematic-stage.png"))

        orch = page.locator(".tdl-v2-region--orchestrator")
        orch.screenshot(path=str(OUT / "phase-5.2-orchestrator-idle.png"))

        sidebar = page.locator(".tdl-v2-region--sidebar")
        sidebar.screenshot(path=str(OUT / "phase-5.2-sidebar-console.png"))

        browser.close()
    httpd.shutdown()
    print("Phase 5.2 screenshots written to", OUT)
