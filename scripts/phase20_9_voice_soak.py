# =====================================
# Titan Phase 20.9 Voice Production Soak CLI
# =====================================

"""Run Phase 20.9 live-provider / enrollment soak scenarios (mock-safe).

Usage (from project root):

    python scripts/phase20_9_voice_soak.py

Writes ``data/phase20_9_soak/report.json``.
"""
...
from voice.production_soak import DEFAULT_SOAK_SCENARIOS, VoiceProductionSoakRunner
...
def main() -> int:
    out_dir = ROOT / "data" / "phase20_9_soak"
    ...
    report = runner.run(scenarios=DEFAULT_SOAK_SCENARIOS)
    path = out_dir / "report.json"
    path.write_text(json.dumps(report.to_dict(), indent=2), encoding="utf-8")
    print(
        f"Phase 20.9 soak ok={report.ok} "
        f"passed={report.to_dict()['passed']}/{report.to_dict()['scenario_count']}"
    )
    print(f"Report: {path}")
