# =====================================
# Phase 4.3 — Pixel-Perfect Cognitive Orchestrator screenshots
# =====================================
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        if phase != "4.3":
            raise RuntimeError(f"Expected data-phase=4.3, got {phase!r}")

        _pin_idle(page)
        page.wait_for_timeout(2200)

        page.screenshot(path=str(OUT / "phase-4.3-orchestrator-full.png"), full_page=False)

        orch = page.locator(".tdl-v2-region--orchestrator")
        box = orch.bounding_box()
        if orch.count():
            orch.first.screenshot(path=str(OUT / "phase-4.3-orchestrator-panel.png"))

        if box:
            page.screenshot(
                path=str(OUT / "phase-4.3-orchestrator-crop.png"),
                clip={
                    "x": max(0, box["x"] - 8),
                    "y": 0,
                    "width": box["width"] + 16,
        ...
        if orch.count():
            orch.first.screenshot(path=str(OUT / "phase-4.3-orchestrator-active.png"))

        browser.close()

    httpd.shutdown()
    print(f"Wrote Phase 4.3 orchestrator screenshots to {OUT}")
