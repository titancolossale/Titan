# =====================================
# Phase 4.2 — Cognitive Orchestrator screenshots
# =====================================
...
def main() -> None:
    ...
    with sync_playwright() as playwright:
        ...
        if "Cognitive Orchestrator" not in title:
            raise RuntimeError(f"Unexpected orchestrator title: {title!r}")

        _pin_idle(page)
        page.wait_for_timeout(2200)

        page.screenshot(path=str(OUT / "phase-4.2-orchestrator-full.png"), full_page=False)

        orch = page.locator(".tdl-v2-region--orchestrator")
        box = orch.bounding_box()
        if orch.count():
            orch.first.screenshot(path=str(OUT / "phase-4.2-orchestrator-panel.png"))

        if box:
            page.screenshot(
                path=str(OUT / "phase-4.2-orchestrator-crop.png"),
                clip={
                    "x": max(0, box["x"] - 8),
                    "y": 0,
                    "width": box["width"] + 16,
        ...
        if orch.count():
            orch.first.screenshot(path=str(OUT / "phase-4.2-orchestrator-active.png"))

        browser.close()

    httpd.shutdown()
    print(f"Wrote Phase 4.2 orchestrator screenshots to {OUT}")
